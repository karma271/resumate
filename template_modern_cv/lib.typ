...
// #let default-accent-color = rgb("#0377fb")
// #let default-accent-color = rgb("#3A8DEF")
          ...
          contact-item(
            (
              ...
              icon: if ("icon" in item) {
                box(fa-icon(item.icon, fill: default-accent-color))
              } else {
                none
              },
...
#let resume(
  ...
  show heading.where(level: 1): it => block(above: 1.2em, sticky: true)[
    ...
    #text(color)[#it.body]
    #box(width: 1fr, line(length: 100%, stroke: (paint: color, thickness: 1.55pt, cap: "round", dash: (array: (0pt, 3pt), phase: 0pt))))
  ]
...
/// Summary/profile section shown at the top of the resume.
/// - body (content): The summary text
...
/// Project entry that flows title, description tag, and body as one inline paragraph.
#let resume-project(title: none, description: "", body) = {
  set par(leading: 0.65em, justify: true)
  block(above: 0.75em, below: 0.75em)[
    #text(size: 8pt, weight: "bold", fill: color-darkgray)[#title]#if description != "" [#text(size: 8pt, weight: "bold", fill: default-sub-accent-color)[ | #description]:] #text(size: 8pt, weight: "light", fill: color-darknight)[#body]
  ]
}
...
#let resume-entry(
  ...
  block(above: 1em, below: 0.65em, sticky: true)[
    #if inline-title {
      [#text(size: 8pt, weight: "bold", fill: color-darkgray)[#title-content]#if description != "" [#text(size: 8pt, fill: default-sub-accent-color)[ | #description]]]
    } else {
      pad[
        #if inline-location {
          let parts = ()
...
#let coverletter(
  ...
  let positions = {
    set text(sub-accent-color, size: 12pt, weight: "regular")
    align(right)[
      #__apply_smallcaps(
